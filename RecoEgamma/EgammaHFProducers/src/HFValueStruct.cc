...
reco::HFValueStruct::HFValueStruct(int& version, std::vector<double>& vect): v_(version),hfvv_(vect) {
  ...
  if(v_==0 || v_==1 || v_==2) doEnCor_=true;
  else  doEnCor_=false;
  
  if(v_==1){ doPU_=true;doPUold_=true;}
  else if(v_==2){ doPU_=true;doPUold_=false;}
  else {doPU_=false,doPUold_=false;}
