...
process.hltPickTriggered = cms.EDFilter('TriggerResultsFilter',
        ...
        triggerConditions       = cms.vstring( 'HLT_Ele15_CaloIdVT_CaloIsoT_TrkIdT_TrkIsoT_v*' ) #pick one!
                                        )
...
## process.p2 = cms.Path(process.hltPickTriggered+process.hltPickTriggered2+process.HFSCfilter+process.patElectronIDs+process.hfRecoEcalCandidate+process.lepton_cands+process.ZFromData2)
process.p1 = cms.Path(process.hltPickTriggered+process.HFSCfilter+process.patElectronIDs+process.hfEMClusteringSequence+process.lepton_cands+process.ZFromData)

	 
process.p2 = cms.Path(process.hltPickTriggered+process.hltPickTriggered2+process.HFSCfilter+process.patElectronIDs+process.hfEMClusteringSequence+process.lepton_cands+process.ZFromData2)
