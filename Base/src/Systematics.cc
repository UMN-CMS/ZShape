...
namespace zshape {
  ...
  // eta-dependent: 0.4% linear with eta along EB, 3% further along TK-covered-EE
  ...
  void EnergyScale::rescale(ZShapeElectron& electron) {
    ...
    if (accept.isInAcceptance(electron,EtaAcceptance::zone_EB)) {
      if (shiftECAL_>0)         isEtaDep_   ?   (scale=1+0.004*absEta/1.50)   : (scale=1.01);
      else if (shiftECAL_<0)    isEtaDep_   ?   (scale=1-0.004*absEta/1.50)   : (scale=0.99);
    } else if (accept.isInAcceptance(electron,EtaAcceptance::zone_EE)) {
      if (shiftECAL_>0)         isEtaDep_   ?   (scale=1.004+0.03*(absEta-1.50))   : (scale=1.03);
      else if (shiftECAL_<0)    isEtaDep_   ?   (scale=0.996-0.03*(absEta-1.50))   : (scale=0.97);
