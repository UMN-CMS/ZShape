...
void myTClass<T>::plot (std::string myfs, RunType &myrt, std::string ops)
{
  ...
  if (myp){
	...
	mypw->Draw(ops.c_str());
      ...
      myp->GetYaxis()->SetTitle("Statistical Error");
...
int main () {
  ...
  myCust_t BlueTri; BlueTri.color = kBlue;  
  BlueTri.marker = kFullTriangleDown; 
  BlueTri.thickness = 1.1; 
  BlueTri.linethick = 2;
  myCust_t RedCirc; 
  RedCirc.color = kRed;  
  RedCirc.marker = kFullCircle;  
  RedCirc.thickness = 0.9; 
  RedCirc.linethick = 3; 
  myCust_t GrnSqur; 
  GrnSqur.color = kGreen;   
  GrnSqur.marker = kFullSquare;  
  GrnSqur.thickness = 0.6; 
  GrnSqur.linethick = 2; 
  myCust_t BlkStar; 
  BlkStar.color = kBlack;  
  BlkStar.marker = kFullStar;  
  BlkStar.thickness = 1.6; 
  BlkStar.linethick = 3; 
  myCust_t CynCirc; 
  CynCirc.color = kCyan;  
  CynCirc.marker = kFullCircle; 
  CynCirc.thickness = 0.9; 
  CynCirc.linethick = 3;
  myCust_t MagnTri; MagnTri.color = kMagenta; 
  MagnTri.marker = kOpenTriangleUp; 
  MagnTri.thickness = 1.3;
  MagnTri.linethick = 2; 
  myCust_t OrngTri; 
  OrngTri.color = kOrange; 
  OrngTri.marker = kOpenTriangleUp; 
  OrngTri.thickness = 1.3; 
  OrngTri.linethick = 3; 
  myCust_t BlkCirc; 
  BlkCirc.color = kBlack;   
  BlkCirc.marker = kFullCircle;  
  BlkCirc.thickness = 1.0; 
  BlkCirc.linethick = 2; 
  ...
  // DARUNMAPCRUST
  myRunMap["ZSummedQuadHist"]["Sum in Quad"]=BlkCirc; 
  myRunMap["WP80.root"]["WP80"]=RedCirc; 
  myRunMap["SC.root"]["Supercluster"]=OrngTri; 
  myRunMap["HLT.root"]["HLT"]=MagnTri; 
  myRunMap["HFSC.root"]["HF Supercluster"]=CynCirc; 
  myRunMap["HFEID.root"]["HF Electron ID"]=GrnSqur; 
  myRunMap["GSF.root"]["GSF Track Match"]=BlueTri; 
  TLegend *myLeg = new TLegend(0.45,0.64,0.65,0.95,"Error From"); 
  std::string legops = "l"; 
  std::string legfops = "l";
  // end sed
