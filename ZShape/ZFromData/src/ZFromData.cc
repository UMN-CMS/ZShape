...
quiet_(iConfig.getUntrackedParameter<bool>("quiet", false)) {
    ...
    allProbeCandTags_ = iConfig.getUntrackedParameter< std::vector<edm::InputTag> >("allProbeCandTags", defaultAllProbeCandTags);
...
void ZFromData::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    try {
        ...
        edm::LogError("ZFromData::GsfElectron ") << "Error! Can't get collection " << gsfProducer_;
        throw ex;
    }
    ...
    if (tagprobes.isValid()) {
        ...
        const int usetp = (tpsize > 0) ? (iEvent.id().event()) % tpsize : 0;
        int tpnum = 0;
    ...
    if (evt_.elec(0).p4_.Pt() < evt_.elec(1).p4_.Pt()) {
    ...
    if (extraHistos_) { 
        allCaseExtra_.Fill(evt_.elec(0), evt_.elec(1), evtMC_.elec(0), evtMC_.elec(1), wgt, doMC_);
    }
    ...
    stdCuts_.dummyCuts(evt_.elec(1));
        ...
        const int necal = (evt_.elec(0).passed("ACC(ECAL+TRK)") ? (1) : (0))+(evt_.elec(1).passed("ACC(ECAL+TRK)") ? (1) : (0));
        const int ntrk = (evt_.elec(0).passed("ACC(ECAL-TRK)") ? (1) : (0))+(evt_.elec(1).passed("ACC(ECAL-TRK)") ? (1) : (0));
        const int nhf = (evt_.elec(0).passed("ACC(HF)") ? (1) : (0))+(evt_.elec(1).passed("ACC(HF)") ? (1) : (0));

        if (necal == 2) {
            accHistos_.ecal_ecal->Fill(pZ.Rapidity());
        } else if (necal == 1 && ntrk == 1) { 
            accHistos_.ecal_ntrk->Fill(pZ.Rapidity());
        } else if (necal == 1 && nhf == 1) { 
            accHistos_.ecal_hf->Fill(pZ.Rapidity());
        } else if (necal == 1) { 
            accHistos_.ecal_noacc->Fill(pZ.Rapidity());
        } else if (ntrk == 2) { 
            accHistos_.ntrk_ntrk->Fill(pZ.Rapidity());
        } else if (ntrk == 1 && nhf == 1) {
            accHistos_.ntrk_hf->Fill(pZ.Rapidity());
        } else if (ntrk == 1) {
            accHistos_.ntrk_noacc->Fill(pZ.Rapidity());
        } else if (nhf == 2) {
            accHistos_.hf_hf->Fill(pZ.Rapidity());
        } else if (nhf == 1) {
            accHistos_.hf_noacc->Fill(pZ.Rapidity());
        } else {
            accHistos_.noacc_noacc->Fill(pZ.Rapidity());
        }
...
evt_.dump();
    ...
    for (std::map<std::string, ZShapeZDef*>::const_iterator q = zdefs_.begin(); q != zdefs_.end(); ++q) {
        ...
        if (!q->second->pass(evt_, 1, 1, 0, &pairing)) { 
            continue;
        }
        int e1n = e1;
        int e2n = e2;
        ...
        if (extraHistos_) {
            plots->acceptanceExtra_.Fill(evt_.elec(e1n), evt_.elec(e2n), evtMC_.elec(e1n), evtMC_.elec(e2n), wgt, doMC_);
        }
...
void ZFromData::fillMCEvent(const HepMC::GenEvent* Evt) {
    ...
    for (HepMC::GenEvent::particle_const_iterator mcpart = Evt->particles_begin(); mcpart != Evt->particles_end(); ++mcpart) {
        const int status = (*mcpart)->status();
        const int pid = (*mcpart)->pdg_id();
        ...
        if (status == 3 && abs(pid) == myPid) {
            ...
            momentum.SetPx((*mcpart)->momentum().x());
            momentum.SetPy((*mcpart)->momentum().y());
            momentum.SetPz((*mcpart)->momentum().z());
            momentum.SetE((*mcpart)->momentum().t());
    ...
    const float pt1 = evtMC_.elec(0).p4_.Pt();
    const float pt2 = evtMC_.elec(1).p4_.Pt();
...
ZFromData::beginJob() {
    ...
    for (std::map<std::string, ZShapeZDef*>::const_iterator q = zdefs_.begin(); q != zdefs_.end(); ++q) {
        ...
        if (extraHistos_) { 
            zplots->acceptanceExtra_.Book(td);
        }
        ...
        for (int i = 1; i < q->second->criteriaCount(ZShapeZDef::crit_E1); ++i) {
            ...
            if (extraHistos_) {
                zplots->postCutExtra_[i - 1].Book(td);
            }
        }
...
int ZFromData::ProbePassProbeOverlap(const reco::CandidateBaseRef& probe, edm::Handle<reco::CandidateView>& passprobes, bool checkExactOverlap_) {
    int ppass = 0;
    if (passprobes.isValid()) {
        for (int ipp = 0; ipp < (int) passprobes->size(); ++ipp) {
            ...
            if (checkExactOverlap_ && isOverlap) {
                ppass = 1;
                continue;
            }
...
bool ZFromData::MatchObjects(const reco::Candidate *hltObj, const reco::CandidateBaseRef& tagObj, const bool exact) {
