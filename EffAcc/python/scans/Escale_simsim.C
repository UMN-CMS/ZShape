...
void Escale_simsim(TFile* f_data, TFile* f_smear,int pm) {
...
float determineChi2(TFile* f_data, TFile* f_smear, TLegend* t1,const FitSmearPoint& pt,int mode) {
  ...
  if (pt.detName=="EB") {
    ...
    sprintf(data_h_name,"mcEff/EB-EB/C07-m(60,120)/Z0_mass_zoom");
    bin_low=41;
    bin_high=80;
  }
  if (pt.detName=="EE") {
    sprintf(smear_h_name,"%s/EE-EB/C08-m(60,120)/Z0_mass_zoom",pt.dirName.c_str());
    sprintf(data_h_name,"mcEff/EE-EB/C07-m(60,120)/Z0_mass_zoom");
    bin_low=41;
    bin_high=80;
  }
